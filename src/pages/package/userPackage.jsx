...
import { GiftOutlined, CheckCircleOutlined, ReloadOutlined } from '@ant-design/icons';
...
function UserPackage() {
  ...
  const paidPackages = packages.filter(pkg =>
    pkg.paymentStatus === 'SUCCESS' && pkg.status !== 'ACTIVE'
  );
  ...
  const handleActivatePackage = async (planId) => {
    // Nếu đã có gói ACTIVE thì không cho kích hoạt thêm
    if (activePackages.length > 0) {
      toast.info('Bạn chỉ được kích hoạt 1 gói duy nhất tại một thời điểm.');
      return;
    }
    ...
    >
      ...
      {options.activate && (
        <Button
          ...
          loading={activatingPlanId === pkg.id} // có thể thêm loading cho đẹp
        >
          Kích hoạt ngay
        </Button>
      )}
    ...
    <div className="min-h-screen bg-white py-10 px-4 md:px-10 lg:px-20">
      <div className="max-w-7xl mx-auto">
        ...
        {activePackages.length > 0 && (
          <>
            <h3 className="text-xl font-semibold mb-2 text-green-700">Gói đang hoạt động</h3>
        ...
        {paidPackages.length > 0 && (
          <>
            <h3 className="text-xl font-semibold mb-2 text-blue-700">Gói đã thanh toán</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
              {paidPackages.map(pkg => renderPackageCard(pkg, { activate: true }))}
            </div>
          </>
        )}
        ...
        {unpaidPackages.length > 0 && (
          <>
            <h3 className="text-xl font-semibold mb-2 text-orange-600">Gói chưa thanh toán</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
              {unpaidPackages.map(pkg => renderPackageCard(pkg, { retry: true }))}
            </div>
          </>
        )}
